...
def convert(mat, SAVE_PATH, NUM_CLASS):
    ...
    id = mat.split('/')[-1][:-4]
    gray_save_name = os.path.join(gray_save_path, id.split("\\")[-1] + '.png')
    color_save_name = os.path.join(color_save_path, id.split("\\")[-1] + '.png')
...
if __name__ == '__main__':
    ...
    NUM_DATA = 10103
    ROOT = 'F:/datasets/PascalContext/trainval/trainval'
    SAVE_PATH = 'F:/datasets/PascalContext/debug'
    NUM_CLASS = 60
    ...
    for f in all_mat:
        index = index +1
        convert(f, SAVE_PATH, NUM_CLASS)
        print("step {}: saved {}".format(index, f))
    end = time.time()
